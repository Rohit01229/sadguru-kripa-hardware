...
apps/admin/
  ...
  messages/
    en/
      common.json  ← `common` namespace (nav, shell, actions, …)
    hi/
      common.json
```
...
- Inside a file, keys may be nested (`nav.dashboard`, `shell.signOut`). Reference the full
  dotted path.
...
The admin nav lives in `app/(admin)/layout.tsx`. `iconKey` (e.g. `dashboard`, `stock`)
doubles as the translation key: labels are resolved server-side with
`t('nav.<iconKey>')` and passed to the client `AdminShell` already localized.

## Rules

- Translate **UI chrome only** — nav, labels, buttons, messages, headings.
- **Do NOT translate user-entered data** (product names, supplier names, the store name
  from `getStoreConfig()`, etc.). Those are data, not strings.
...
The `LanguageSwitcher` (`app/(admin)/LanguageSwitcher.tsx`, an `EN | हिं` toggle) writes
the `NEXT_LOCALE` cookie (path=/, ~1yr) and calls `router.refresh()` so server components
re-render in the new locale — no full reload. It sits in the admin topbar (`AdminShell.tsx`).
